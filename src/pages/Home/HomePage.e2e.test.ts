...
test.describe('HomePage E2E Tests', () => {
    ...
    test.describe('Page Rendering', () => {
        ...
        test('доступен через прямую навигацию', async ({ page }) => {
    ...
    test.describe('File Upload - Browse', () => {
        test('грузит CSV файл через файловый браузер', async ({ page }) => {
        ...
        test('обрабатывает валидацию не-CSV файлов', async ({ page }) => {
            const fileInput = page.locator('input[type="file"]');
            await fileInput.setInputFiles([
                {
                    name: 'test.txt',
        ...
        test('вызывает файловый браузер при клике на кнопку загрузки', async ({ page }) => {
    ...
    test.describe('File Upload - Drag & Drop', () => {
        test('обрабатывает состояния перетаскивания', async ({ page }) => {
        ...
        test('грузит CSV файл через перетаскивание', async ({ page }) => {
            await page.evaluate(() => {
        ...
        test('отклоняет не-CSV файлы при перетаскивании', async ({ page }) => {
            await page.evaluate(() => {
                const dropzone = document.querySelector('[data-testid="dropzone"]');
    ...
    test.describe('CSV Analysis Flow', () => {
        test('успешно анализирует CSV файл', async ({ page }) => {
        ...
        test('обрабатывает ошибки анализа корректно', async ({ page }) => {
            await page.route('**/aggregate?rows=10000', async (route) => {
                await route.fulfill({
                    status: 500,
    ...
    test.describe('Clear Functionality', () => {
        test('очищает файл и сбрасывает состояние', async ({ page }) => {
            const fileInput = page.locator('input[type="file"]');
            await fileInput.setInputFiles([
                {
                    name: 'clear-test.csv',
        ...
        test('очищает хайлайты после успешного анализа', async ({ page }) => {
            await page.route('**/aggregate?rows=10000', async (route) => {
                const mockData = {
    ...
    test.describe('User Experience', () => {
        test('предотвращает операции с файлами во время обработки', async ({ page }) => {
            await page.route('**/aggregate?rows=10000', async (route) => {
        ...
        test('показывает соответствующие сообщения для разных состояний', async ({ page }) => {
    ...
    test.describe('Accessibility', () => {
        test('доступен с клавиатуры', async ({ page }) => {
